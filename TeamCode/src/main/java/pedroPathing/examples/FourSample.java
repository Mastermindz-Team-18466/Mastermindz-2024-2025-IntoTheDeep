...
@Config
@Autonomous(name = "Four Sample", group = "Examples")
public class FourSample extends OpMode {
    ...
    private final Pose one = new Pose(5, 14, Math.toRadians(-5));
    private final Pose two = new Pose(5, 14, Math.toRadians(14.2));
    ...
    public void autonomousPathUpdate() {
        switch (pathState) {
            ...
            case 26:
                ...
                break;
            case 27:
                if(pathTimer.getElapsedTimeSeconds() > 0.5 && intakeOuttake.claw.claw.getPosition() > 0.8 && follower.getPose().getX() > (three.getX() - 2) && follower.getPose().getY() > (three.getY() - 2)) {
                    intakeOuttake.setInstructions(IntakeOuttake.Instructions.HOLD);
                    intakeOuttake.setSpecificInstruction(IntakeOuttake.SpecificInstructions.MAX_RETRACT);
                    setPathState(28);
                }
